      ...
      {showArchiveModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.9)", zIndex: 9999, display: "flex", justifyContent: "center", alignItems: "center", padding: 20 }}>
          <div className="az-card" style={{ width: "100%", maxWidth: 600, background: "#111" }}>
            <div className="az-card-inner">
              <h3 style={{ color: "var(--brand-orange)", marginBottom: 20 }}>🗄️ Arquivo de Oportunidades</h3>
              <div style={{ maxHeight: "60vh", overflowY: "auto", display: "grid", gap: 12 }}>
                {ocultosAgrupamento.length === 0 ? <p className="az-muted">O arquivo está vazio.</p> : 
                  oportunidadesCNE.filter(op => ocultosAgrupamento.includes(op.id)).map(op => (
                    <div key={op.id} style={{ background: "rgba(255,255,255,0.05)", padding: 12, borderRadius: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div><div style={{ fontWeight: 700, color: "white" }}>{op.titulo}</div><div className="az-small muted">{op.coluna}</div></div>
                      {!readOnly && <button className="az-btn az-btn-teal" style={{ padding: "4px 12px" }} onClick={() => handleRecuperarPadlet(op.id)}>♻️ Recuperar</button>}
                    </div>
                  ))
                }
              </div>
              <div style={{ marginTop: 24, textAlign: "right" }}><button className="az-btn" onClick={() => setShowArchiveModal(false)}>Fechar</button></div>
            </div>
          </div>
        </div>
      )} 
              ...
              <form onSubmit={handleShareSubmit}>
                  ...
                  <div style={{ display: "grid", gap: 8, background: "rgba(0,0,0,0.2)", padding: 12, borderRadius: 8 }}>
                    {[
                      ...
                      <label key={alvo.id} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
                        <input
                          type="checkbox"
                          checked={shareTargets.includes(alvo.id)}
                          onChange={() => handleTargetToggle(alvo.id)}
                        />
                ...
                <div className="az-form-group" style={{ marginBottom: 20 }}>
                  ...
                  <input
                    ...
                    className="az-input"
                    value={shareDataFim}
                    onChange={e => setShareDataFim(e.target.value)}
                  />
                </div>
