/* Grelha do Guia:
    Vê o que está pendente e consegue fazer as ações do workflow:
    Confirmar (quando está VALIDADO ou quando é ESCOLHA com autoGuia=true)
    Recusar (volta para RECUSADO, desbloqueia o elemento para propor de novo)
    Concluir (quando está REALIZADO)
    ./pages/ChefeUnidadeObjetivos.jsx
...
export default function ChefeAgrupamentoDashboard({ profile }) {
  ...
  async function fetchDadosAgrupamento() {
    ...
    try {
      ...
      const listaDir = [];
      // Filtro limpo na mesma linha: só adiciona se ativo !== false
      snapDir.docs.filter(d => d.data().ativo !== false).forEach(d => listaDir.push({ uid: d.id, ...d.data() }));
      ...
      const listaElem = [];
      // Filtro limpo na mesma linha: só adiciona se ativo !== false
      snapElem.docs.filter(d => d.data().ativo !== false).forEach(d => listaElem.push({ uid: d.id, secaoDocId: d.data().secaoDocId || "Outra" }));
